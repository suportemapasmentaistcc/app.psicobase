...
async function validateUser(auth){if(!SUPABASE_URL||!SUPABASE_ANON_KEY||!auth?.startsWith('Bearer '))return null;const r=await fetch(`${SUPABASE_URL}/auth/v1/user`,{headers:{apikey:SUPABASE_ANON_KEY,Authorization:auth}});if(!r.ok)return null;return r.json();}
async function callModel(messages,max_tokens=4200,temperature=.25){const models=['deepseek/deepseek-chat-v3-0324:free','qwen/qwen3-235b-a22b:free','meta-llama/llama-3.3-70b-instruct:free','openrouter/free'];let last=null;for(const model of models){try{const r=await fetch('https://openrouter.ai/api/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${OPENROUTER_API_KEY}`,'HTTP-Referer':'https://app-psicobase.vercel.app','X-Title':'PsicoBase'},body:JSON.stringify({model,temperature,max_tokens,messages})});const raw=await r.json();if(r.ok&&raw?.choices?.[0]?.message?.content)return raw;last=raw}catch(e){last=e}}console.error(last);throw new Error('MODEL_UNAVAILABLE')}
...
module.exports=async function(req,res){
 ...
 try{
  const authUser=await validateUser(req.headers.authorization); if(!authUser)return send(res,401,{error:'Entre novamente para continuar.'});
  if(!OPENROUTER_API_KEY)return send(res,500,{error:'O serviço de elaboração ainda não foi configurado.'});
  const {topic,level='Intermediário',goal='Compreender o conceito',memory=[]}=req.body||{};
  ...
  const user=`TEMA SOLICITADO: ${safeTopic}\nNÍVEL: ${String(level).slice(0,40)}\nOBJETIVO: ${String(goal).slice(0,80)}\n\nBASE CURADA DA PSICOBASE:\n${retrieved.text}\n\nMEMÓRIA RECENTE DO ALUNO: ${JSON.stringify(Array.isArray(memory)?memory.slice(-12):[])}\nCrie uma sessão de estudo coerente, com 4 questões discursivas, 4 perguntas orais e um exercício clínico breve. Quando pertinente, conecte ao percurso anterior do aluno. As referências devem ser apenas as já presentes na base.`;
